...
export const LabSwapBlock: FC<Props> = ({visible, size, inline, edge, bottom, data}) => {
  ...
  const [requestEnabled, setRequestEnabled] = useState<boolean>(false);
  const [offerEnabled, setOfferEnabled] = useState<boolean>(false);
  ...
  if (isScheduledFor !== -1 && course_instance.section_ids.length > 1) {
    buttonRequest = <button id={id} disabled={!requestEnabled} request-type="request" onClick={selectBlockForTrade} className="submit-button">Request selected section</button>
    buttonOffer = <button id={id} disabled={!offerEnabled} request-type="offer" onClick={selectBlockForTrade} className="submit-button">Offer selected section</button>
