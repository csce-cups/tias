...
export const LabSwap = () => {
  ...
  const renderSwapSets = (
    retData: TradeRequest[],
    action: "Outstanding" | "Pending" | null,
    filter: (request: TradeRequest) => boolean
  ) =>
    retData.filter(filter).map((request) => {
      const allBlocks = renderScheduled(viableBlockWeekLocal); // For easier iteration
      ...
      if (action !== null) {
        if (action === "Pending") {
          //we sent this out, action is cancel
          actions = (
            <div>
          ...
          //incoming requests, so actions are accept and reject
          actions = (
            <div>
                ...
                <Scrollable>
                  <div className="swap-section vstack">
                    ...
                    {renderSwapSets(
                      ...
                      "Pending",
                      (request) =>
                        request.person_id_receiver === userId &&
                  ...
                  <div className="swap-section vstack">
                    ...
                    {renderSwapSets(
                      ...
                      "Outstanding",
                      (request) =>
                        request.person_id_sender === userId &&
                        request.request_status === "Pending"
                    )}
                  </div>
