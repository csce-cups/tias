...
exports.handler = async (event) => {
    ...
    for (let {person_id, email} of people_in_requested) {
        ...
        const viableCourses = await helper_functions.queryDB(`SELECT * FROM "ViableCourses"($1) WHERE section_id = $2`, [person_id, offered_id]).catch(err => helper_functions.GenerateErrorResponseAndLog(err, response, "Failed to get the viable courses for at least one person in the requested section."))
