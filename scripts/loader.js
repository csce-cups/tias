...
const update_course_sections = (dbobj, section_id, section_number, weekday, start_time, end_time, place, course_type) => {
    course_sections.push({course_id: dbobj.course_id, department: dbobj.department, course_number: dbobj.course_number, course_name: dbobj.course_name, section_id: section_id, person_id_professor: null, section_number: section_number, peer_teachable: null, weekday: weekday_map[weekday], start_time: to_db_time(start_time), end_time: to_db_time(end_time), place: place, meeting_type: course_type_map[course_type]})
}

const add_section_meetings = (modifications, weekdays, section_id, start_time, end_time, place, course_type, dbobj, section_number) => {
    for (let day of weekdays) {
        modifications.push(loader(`
        ...
        update_course_sections(dbobj, section_id, section_number, day, start_time, end_time, place, course_type)
    }
}
...
client.connect()
    ...
    .then(() => {if (DEBUG) console.log('Finished All Loads.')})
    .then(() => client.end())
